...
//425,690,689""".lines()
    ...
    edges.sortedBy { it.third }.take(1000).forEach {
        dsu.connect(it.first, it.second)
    }

    val szs = dsu.components.indices
        .map { dsu.find(it) }.toSet().toMutableList()
        .sortedByDescending { dsu.size[it] }
    println(szs.take(3).fold(1L) { acc, s -> acc * dsu.size[s] })
}
